...
| Adoption handover photo                      | May show a home or a family                    | Storage path only; a short-lived (1 hour) signed URL is generated at read time and never persisted |
| Animal primary photo                         | Identifies the animal, not a person            | Storage path only; a short-lived (1 hour) signed URL is generated at read time and never persisted |
...
- **Replacing a persisted photo does not delete the previous Storage object.** An orphaned object is
  accepted as harmless for V1; garbage collection is deferred. This is distinct from
  [ADR-013](decisions/013-preserve-history-and-restrict-deletion.md), which governs historical
  domain records, not superseded image blobs.
...
| **Unauthorized image access**                       | A follow-up photo of an adopter's home is readable by another shelter                                                                             | Private `shelter-media` bucket and shelter-scoped `storage.objects` policies in the migrations (no automated test; see [ADR-026](decisions/026-remove-local-supabase-test-stack.md)); `set_animal_primary_photo`, `set_adoption_photo`, and `set_followup_photo` each re-validate the shelter prefix server-side as defense in depth                                               | Implemented         |
| **Dangling photo reference**                        | A caller invokes a photo-attach RPC directly with a path that was never uploaded, or an upload fails between the object write and the attach call | `set_animal_primary_photo`, `set_adoption_photo`, and `set_followup_photo` each require `p_path` to name an object that exists in `shelter-media` before recording it, in addition to the shelter-prefix check; a client-side upload-then-attach order is not treated as a substitute for a server-side precondition ([ARCHITECTURE.md](ARCHITECTURE.md#atomic-domain-operations)) | Implemented         |
...
- **Storage bucket, shelter-scoped policies, and persisted photo paths are implemented.** The
  private `shelter-media` bucket, `storage.objects` policies, client upload path, signed-URL reads,
  and attach RPCs are in place. Hosted two-shelter and physical-device validation remain pending.
