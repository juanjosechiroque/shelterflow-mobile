...
- **Replacing the animal primary photo does not delete the previous Storage object.** An orphaned
  object is accepted as harmless for V1; garbage collection is deferred. This is distinct from
...
| **Unauthorized image access**                       | A follow-up photo of an adopter's home is readable by another shelter                                      | Private `shelter-media` bucket and shelter-scoped `storage.objects` policies in the migrations (no automated test; see [ADR-026](decisions/026-remove-local-supabase-test-stack.md)); `set_animal_primary_photo` re-validates the shelter prefix server-side as defense in depth; a real two-shelter hosted-session re-confirmation for this path is still pending | Implemented for the animal primary photo path, pending its manual two-shelter re-confirmation; adoption handover and follow-up photos still pending (phase 8, slices 8.3/8.4) |
