...
export function createPrototypeFlowReducer(clock: Clock) {
  return function prototypeFlowReducer(
    ...
    switch (action.type) {
      case 'RECORD_EVALUATION': {
        ...
        const recordedOn = clock.todayISO();
        const evaluation = {
          ...action.evaluation,
      ...
      case 'MARK_DECISION_PENDING': {
        ...
        if (!candidate || candidate.status !== 'MEETING_SCHEDULED')
          return state;
